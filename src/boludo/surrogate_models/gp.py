...
class GP(SingleTaskGP):
    def __init__(
        ...
        likelihood = likelihood if likelihood else self._get_likelihood()
    ...
    def _get_likelihood(self):
        noise_prior = GammaPrior(1.1, 0.5)  # 0.05
    ...
    def _fix_noise(self):
        ...
        self.covar_module.register_prior("outputscale_prior", output_scale_prior, "raw_outputscale")
        self.likelihood.noise_covar.register_prior(
            "noise_prior", likelihood_noise_prior, "raw_noise"
        )

        self.covar_module.raw_outputscale.constraint = gpytorch.constraints.Interval(-10.0, 10.0)
        self.likelihood.noise_covar.raw_noise.constraint = gpytorch.constraints.Interval(-5.0, 5.0)
    ...
    def transform_data(self, train_x, train_y):
        if hasattr(self, "outcome_transform") and self.outcome_transform:
            train_y, _ = self.outcome_transform(train_y)
        if hasattr(self, "input_transform") and self.input_transform:
            train_x = self.input_transform(train_x)
    ...
    def fit(self, train_x, train_y, state_dict=None):
        ...
        Args:
            ...
            state_dict: current state dict used to speed up fitting
        """
        train_x, train_y = self.transform_data(train_x, train_y)
        ...
        for i in range(max_retries):
            try:
                ...
                if i == max_retries - 1:  # If we're on our last try
                    raise RuntimeError(f"Failed to fit after {max_retries} retries.") from e
...
class SimpleGP(SurrogateModel, SingleTaskGP):
    def __init__(
        ...
        self.likelihood.noise_covar.register_constraint("raw_noise", GreaterThan(noise_constraint))
